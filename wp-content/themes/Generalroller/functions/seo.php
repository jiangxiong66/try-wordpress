...
function edit_category_seo_format($tag){  
      wp_enqueue_media();
		...
		if(get_option('images_'.$tag->term_id)){$img='<img style="max-width:600px;" src="'.get_option('images_'.$tag->term_id).'" />';}
			if(get_option('images_m_'.$tag->term_id)){$img_m='<img style="max-width:600px;" src="'.get_option('images_m_'.$tag->term_id).'" />';}
   $categorys = get_categories(array('hide_empty' => 0));	
   ...
   <div class="form-field">  
          ...
          <th scope="row"><label for="cat_nummber">这个分类显示多少数量？</label>  </th>  
            <td> 
		 <input name="cat_nummber" id="cat_nummber" type="text" value="'. get_option('cat_nummber_'.$tag->term_id).'" size="10">  <br />
...
function theme_keyworeds(){
   ...
   $cat_keword_p=get_option('cat_keword_p_'.$term_id);
	...
	if ($detect->isMobile()||$detect->isTablet()){
		 $theme_keword= get_option('mytheme_keywords_p');}
	  ...
	  elseif( is_page()||is_single()) {
		  $posttags = get_the_tags();
			 if ($posttags) { foreach($posttags as $tag) { $theme_kewordss.=$tag->name.',';}}
			if($detect->isMobile()||$detect->isTablet()){
			
			if(get_post_meta($id, "关键字_p",true)){ $theme_keword= get_post_meta($id, "关键字_p",true);} 
			  elseif(get_post_meta($id, "关键字",true)){ $theme_keword= get_post_meta($id, "关键字",true);}elseif(get_option('mytheme_keywords_p')){$theme_keword= get_option('mytheme_keywords_p'); }elseif($theme_kewordss){$theme_keword=$theme_kewordss;}else{$theme_keword= get_option('mytheme_keywords');}
		}else{
	...
	elseif(is_category()){
		if($detect->isMobile()||$detect->isTablet()){
			...
			if($cat_keword_p){$theme_keword= $cat_keword_p;} elseif($cat_keword){ $theme_keword= $cat_keword;}elseif(get_option('mytheme_keywords_p')){$theme_keword= get_option('mytheme_keywords_p');}else{$theme_keword= get_option('mytheme_keywords');}
		}else{
...
function theme_description(){
   ...
   $cat_description_p=get_option('cat_description_p_'.$term_id);
      if(is_front_page() || is_home()) { 
	if ($detect->isMobile()||$detect->isTablet()){
		 $theme_description= get_option('mytheme_description_p');}
	  ...
	  $excerp= preg_replace('/\[.*?\]/', '', $excerpk);
			...
			if(get_post_meta($id, "描述_p",true)){ $theme_description= get_post_meta($id, "描述_p",true);} 
			  elseif(get_post_meta($id, "描述",true)){ $theme_description= get_post_meta($id, "描述",true);}elseif(get_option('mytheme_description_p')){$theme_description= get_option('mytheme_description_p');}elseif($excerp){$theme_description=$nokongex;}else{$theme_description= get_option('mytheme_description');}
		}else{
	...
	elseif(is_category()){
		if($detect->isMobile()||$detect->isTablet()){
			...
			if($cat_description_p){$theme_description= $cat_description_p;} elseif($cat_description){ $theme_description= $cat_description;}elseif(get_option('mytheme_description_p')){$theme_description= get_option('mytheme_description_p');}else{$theme_description= get_option('mytheme_description');}
		}else{
