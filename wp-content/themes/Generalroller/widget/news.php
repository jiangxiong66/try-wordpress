...
<em style="padding:3px; background:#FCF3E4; border:solid 1px #F0D8BF; display:block;">由于这款主题的模块可以显示在不同的区域，因此不同的区域的seo标签是可以自定义的，以此增强你的权重递归性，如果不明白如何使用，可以<a href=" http://www.themepark.com.cn/wordpresswzseobqdgxjy.html" target="_blank">点击查看分析文章</a></em>
	...
	extract( $args );
	 ...
	 $args = array( 'cat' => $w_cat , 'showposts' => $nnmber2, 'post__in' =>$post__in );     $query = new WP_Query( $args );  
	 	if($my_b_images){
 ...
 if ($detect->isMobile()){$nunbersss="80";}else{$nunbersss="120";}
 ?> 
...
<div class="about_ttile_line">
 ...
 if(is_home()||is_page()&&$donghuaopens){  if(!get_option("mytheme_donghuaopen")){?> <script> $(window).scroll(function () {$(".donghuaopen .news_tuoch").news();}); </script>  
        <?php
	}}}
}
