...
function extraordinaryvision_customize_css(){
 $detect = new Mobile_Detect();
	...
	$index_blue2=get_option('mytheme_index_blue2');
...
$footer_grel=get_option('mytheme_footer_grel');
	...
	$mytheme_nav_bac_img=get_option('mytheme_nav_bac_img');
...
<style id="extraordinaryvision_customize_css" type="text/css">
	 <?php 
				   ...
				   if($index_blue2!="#f50028"){echo '.mantitle, .mantitle a,.about_btn a{color:'.$index_blue2.' }.about_ttile_line .about_title_line_in,.about_btn a:hover{background:'.$index_blue2.';} .about_btn a{border:1px solid '.$index_blue2.'}';}
				 ...
				 if( $mytheme_nav_hover&&$mytheme_nav_hover!='#fafafa'){echo '#header_pic_nav li:hover, #header_pic_nav li.current-menu-item{ background:'.$mytheme_nav_hover.';}';}
				  if( $mytheme_nav_hover_boder&&$mytheme_nav_hover_boder!='#ffa800'){echo '#header_pic_nav li.current-menu-item a{ border-bottom:3px solid '.$mytheme_nav_hover_boder.';}';}
				   if( $mytheme_nav_title&&$mytheme_nav_title!='#333'){echo '#header_pic_nav li div{ color: '.$mytheme_nav_title.';}';}
				  if( $mytheme_nav_title2&&$mytheme_nav_title2!='#ffa800'){echo '#header_pic_nav li p{ color: '.$mytheme_nav_title2.';}';}
				    if( $mytheme_title_hover){echo '#header_pic_nav li:hover div, #header_pic_nav li.current-menu-item div,#header_pic_nav li:hover p, #header_pic_nav li.current-menu-item p{ color: '.$mytheme_title_hover.';}';}
				   if( $mytheme_top_title&&$mytheme_top_title!='#ffa800'){echo '.header_bottom_right a{ color: '.$mytheme_top_title.';}';}
	...
	echo stripslashes(get_option('mytheme_zdycss'));
	  ...
	  if($detect->isMobile()&&!is_home()&&get_post_meta($id, "customize",true)!='modle1'){echo '.header{top:7px;}';}
