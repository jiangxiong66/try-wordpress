...
class up_down extends WP_Widget {
	...
	function up_down()
	{
		...
		parent::WP_Widget($id_base="up_down",$name='上下图文模块【自定义排版模块】',$widget_ops,$control_ops);  
...
<?php    wp_enqueue_media(); ?>
 ...
 <div  id="up_down_mod" class="swiper-slide  <?php if ($first_mod == "1" ) {echo "swiper-slide-active";}?> up_down" > 
                  ...
                  <?php if($my_images&&$left_right){ ?>
                   <a target="_blank" <?php if($my_url) {echo'href="'.$my_url.'"';} ?>  class="down_pic" <?php echo $up_xy; ?> >
      ...
      <div class="index_mod_bag <?php if(!$my_b_images_ad){echo 'bag_animate'; } ?> " style=" background-image:url(<?php if($my_b_images){echo $my_b_images ;}else{ echo get_bloginfo('template_url').'/images/demo/2.jpg'; }?>)">  <?php if( $my_mp4) {?> <video autoplay="true" loop="true" preload="auto" muted="muted"  src="<?php echo $my_mp4 ; ?>"></video><?php } ?></div> 
             </div>
