...
class left_right extends WP_Widget {
	...
	function left_right()
	{
		...
		parent::WP_Widget($id_base="left_right",$name='左右图文模块【自定义排版模块】',$widget_ops,$control_ops);  
...
<p>
 ...
 <em style="padding:3px; background:#FCF3E4; border:solid 1px #F0D8BF; display:block;">背景视频会在ie10以上浏览器上自动播放手机上不显示（手机上不能自动播放视频，而会出现一个播放按钮），尺寸为1280*720,并且必须为MP4格式</em>
</p> 
...
<p style="display:block; width:100%; border-bottom:1px #333333 solid; padding:5px; margin:5px;"><strong>文字模块设置</strong></p>
 ...
 <p>
 <label  for="<?php echo $this->get_field_id('my_text2'); ?>">文字模块-第二行标题:</label>
...
<?php    wp_enqueue_media(); ?>
 ...
 <div  id="left_right_mod" class="swiper-slide <?php if ($first_mod == "1" ) {echo "swiper-slide-active";}?> left_right" > 
                 
                  <div class="swiper-slide_in">
                  <?php if($my_images){ ?>
                   <a  target="_blank" <?php if($my_url) {echo'href="'.$my_url.'"';} ?>  class="left_pic" <?php if ($left_right == "1" ) {echo "id='right_posib'";}else{echo "id='left_posib'";};  echo $myimages_xy;?>>
                   ...
                   <div class="right_text <?php echo $my_text_colors; ?>"<?php if ($left_right == "1" ) {echo "id='left_posib'";}else{echo "id='right_posib'";}; echo $mytext_xy;?> >
                      ...
                      <?php if( $my_text4) {?>  <a  target="_blank" href="<?php echo  $my_url; ?>" class="btn buy"><b><?php echo  $my_text4; ?></b><div class="btn_bag"></div></a><?php } ?>
                   </div>
                  </div>
                   
                   
      <div class="index_mod_bag <?php if(!$my_b_images_ad){echo 'bag_animate'; } ?> " style=" background-image:url(<?php if($my_b_images){echo $my_b_images ;}else{ echo get_bloginfo('template_url').'/images/demo/4.jpg'; }?>)">  <?php if( $my_mp4) {?> <video autoplay="true" loop="true" preload="auto" muted="muted"  src="<?php echo $my_mp4 ; ?>"></video><?php } ?></div> 
             </div>
